class Solution:
	'''
		...
		input: list[int] = count
			count: range(0, 225)
			count[k]: Frequenzy of "k" 	(i.e.: number of times "k" has appared in the sample)
		output: list[float] = [minimum, maximum, mean, median, mode]
			NOTE: answers have a error margin of: 10^-5 	(i.e.: Answers within 10-5 of the actual answer will be accepted)

	'''
	

	# def sampleStats(self, count: list[int]) -> list[float]:
	# 	self.count = count #incase
	# 	# self.getMinimum()
	# 	print("count sorted:")
	# 	print(sorted(count))
	# 	lenCount = len(count)
	# 	# iterate the list:
	# 	min=0 #sets it as the first value as a starting point
	# 	max=0 #sets it as the first value as a starting point
	# 	i = 0 #number of that 'count[k]' is counting
	# 	k = 0 #(i.e.: count[k]) the count of the number 'i' represents
	...
	# 	median_point = 0 #> MEDIAN alt 1:
	# 	# median_counter = lenCount #> MEDIAN alt 2:
	# 	median_counter = 0
	...
	# 	while k < len(count):
	# 		''' in this case i (index) will also represent the number that count[k] is counting.'''
	# 		#getting min: the first count above 0 in list.
	# 		if not min and count[k]>0:
	# 			min = i

	# 		#getting max: the last count above 0 in list.
	# 		#? maybe itterate the other way for efficiancy then we can reuse the same formula for min
	# 		if count[k]>0:
	# 			max = i

	# 		#mean 1/2: sum((every number above 0) * count) / len() --> finish at the end
	# 		if count[k]>0:
	# 			mean_counter += count[k]
	# 			mean_sum += count[k]*i

	# 		#! dropping median for now 
	# 		# #median 1/2: counting all actual numbers (getting the actual index), thn finding the halfway point --> count[median_point]
	# 		# if count[k]>0:
	# 		# 	median_counter += count[k]-1 #since 1 iteration of count[k] has already been counted.
	# 		# 	# median_point = median_counter/2 #for after the loop

	# 		# mode: the one with the biggest count
	# 		if count[k]>freq_max:
	# 			freq_max=count[k]
	# 			mode=i
	...
	def sampleStats(self, count: list[int]) -> list[float]:
		...
		for i, k in count_dict.items():
			# Check if mid1 is in this block
			if a is None and acc + k > mid1:
				a = i
			# Check if mid2 is in this block
			if b is None and acc + k > mid2:
				b = i
			acc += k
			if a is not None and b is not None:
				break
		# Set median value after calculation
		if len_count % 2:
			median = float(b)
		else:
			median = (a + b) / 2
